...
class Module:
    ...
    def __post_init__(self):
        self.include_patterns = [p.replace(".class", ".java") for p in self.include_patterns]
    ...
    def find_tests(self, test_subdir: Path) -> Iterable[Tuple[Path, str, str]]:
        excluded = {file for exclude in self.exclude_patterns for file in test_subdir.glob(exclude)}
    ...
    def find_cuts(self, tests: Iterable[Tuple[Path, str, str]], src_subdir: Path) -> Iterable[CutPair]:
    ...
    def get_cut_name(test_path: Path, include_pattern: str) -> str:
        ...
        if include_pattern.endswith("/*Test.java"):
            ...
            return re.sub(r"(?:Test)?(.*?)(?:Test|TestCase)?\.java", r"\1.java", test_path.stem)
